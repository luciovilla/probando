...
```
cd probando
mkvirtualenv --no-site-packages probando
...
```
* * * * * ubuntu bash /home/ubuntu/apps/probando/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|probando|tweet|`location`||||
|probando|facebook|`location`||||
|probando|email|`location`||||
|probando|new-comment||||
|probando|open-share-discuss||||
|probando|close-share-discuss||||
|probando|summary-copied||||
|probando|featured-tweet-action|`action`||``tweet_url``|
|probando|featured-facebook-action|`action`||``post_url``|
